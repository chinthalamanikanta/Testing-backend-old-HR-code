...
public interface TaskJpaRepository extends JpaRepository<Task, Integer> {
  ...
  @Query("SELECT t FROM Task t WHERE t.taskAssignedById = :taskAssignedById AND t.taskStatus = false AND t.dueDate < CURRENT_DATE")
  List<Task> allOverDueTasksAssignedFrom(@Param("taskAssignedById") String taskAssignedById);

  @Query("SELECT t FROM Task t WHERE t.taskAssignedById = :taskAssignedById AND t.taskStatus = false AND (t.dueDate >= CURRENT_DATE OR t.dueDate IS NULL)")
  ...
  @Query("SELECT t FROM Task t WHERE t.personId = :personId AND t.taskStatus = false AND t.dueDate < CURRENT_DATE")
  List<Task> allOverDueTasksPersonId(@Param("personId") String personId);

  @Query("SELECT t FROM Task t WHERE t.personId = :personId AND t.taskStatus = false AND (t.dueDate >= CURRENT_DATE OR t.dueDate IS NULL)")
